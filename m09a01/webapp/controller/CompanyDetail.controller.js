sap.ui.define([
    ...
    return Controller.extend("moovi.m09a01.controller.CompanyDetail",{
        ...
        handleSuccessSave: function(oRes, oData){
            ...
            var oModel = this.getView().getModel()
            if(oRes._batchResponse){
                if(oRes._batchResponse[0].response){
                    var status = parseInt(oRes._batchResponse[0].response.statusCode)

                    if(status >= 400){

                        var oResponseBody = JSON.parse(oRes._batchResponse[0].response.body)
                ...
                } else if (oRes.__barchResponses[0].__changeResponses){

                    var aChangeRes = oRes.__changeResponses[0].__changeResponses
